...
class SurvivalModeGameManager:
	...
	def draw_whole_field(self):
		...
		print(self.centralize_score_n_time + "P1: " +
			str(self.score_1) + ' | ' +	'Time: ' +
			str(round(time.time() - self.time)) + ' | ' + 
			'P2: ' + str(self.score_2))
	...
	def process_common_logic_of_2_snakes(self):
		'''Method adds some logic because of increased number of 
		snakes in one game field'''
